...
const NAV = [
  { to: '/employe/accueil',    icon: '🏠', label: 'Accueil' },
  { to: '/employe/calendrier', icon: '📅', label: 'Agenda' },
  { to: '/employe/scanner',    icon: '📷', label: 'Scanner' },
  { to: '/employe/walkin',     icon: '➕', label: 'Walk-in' },
  { to: '/employe/stats',      icon: '📊', label: 'Stats' },
]

export default function EmployeLayout() {
  const { employe, user } = useAuthStore()
      ...
      <header style={{ position: 'sticky', top: 0, zIndex: 50, background: `rgba(250,250,248,0.96)`, backdropFilter: 'blur(10px)', borderBottom: `1px solid rgba(14,12,9,0.08)`, padding: '0 16px' }}>
        <div style={{ height: '56px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            {employe?.couleur_agenda && (
              <div style={{ width: '10px', height: '10px', borderRadius: '50%', background: employe.couleur_agenda }} />
            )}
            <span style={{ fontSize: '13px', color: NOIR, fontWeight: 500 }}>{user?.prenom || `Employé`}</span>
          </div>
        </div>
      </header>
      <main><Outlet /></main>
